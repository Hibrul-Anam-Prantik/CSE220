...
public class BinaryTree
{
    ...
    public BTNode createTreeFromArray(int[] arr, int i) {
    ...
    // Depth of a node
    public int depth(BTNode root, int key, int d) {
    ...
    public boolean isFull(BTNode node) {
    ...
    public boolean isComplete(BTNode node, int index, int totalNodes) {
        if (node == null) return true;
    ...
    public boolean isPerfect(BTNode node, int depth, int level) {
        if (node == null) return true;
    ...
    private int checkBalance(BTNode node) {
        if (node == null) return 0;
